...
def getVideoAnalytics(video_url):
    ...
    if video_data["items"]:
        ...
        video_info['month'] = pd.to_datetime(video_date['date']).month
        video_info['is_weekend'] = 1 if pd.to_datetime(video_date['date']).weekday() >= 5 else 0
        ...
        video_info['watch_per_view'] = video_info['watch_time_minutes'] / int(views) 
        ...
        #     'month': video_info['month'],
        #     'is_weekend': video_info['is_weekend'],
        #     'engagement_rate': video_info['engagement_rate'],
        #     'watch_per_view': video_info['watch_per_view']
        #     }
...
with col2:
    ...
    menu = option_menu(
        menu_title=None,
        options=[ "Linear Regression", "Ridge Regression", "Lasso Regression"],
    ...
    if menu == "Linear Regression":
        ...
        if output is not None:
            with st.container():
                ...
                prediction = lr.predict(video_df_scaled)
        ...
        if output is not None:
            with st.container():
                ...
                prediction = ridge.predict(video_df_scaled)
                st.subheader('Predicted Ad Revenue (USD)')
                data_frame=pd.DataFrame(prediction,columns=['Ad Revenue (USD)'])
                st.write(f"${prediction[0]:.2f}")
                # st.subheader('Video Information')
        ...
        if output is not None:
            with st.container():
                ...
                prediction = lasso.predict(video_df_scaled)
                st.subheader('Predicted Ad Revenue (USD)')
                data_frame=pd.DataFrame(prediction,columns=['Ad Revenue (USD)'])
                st.write(f"${prediction[0]:.2f}")
                # st.subheader('Video Information')
